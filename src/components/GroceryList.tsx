...
interface GroceryListProps {
  items: GroceryItem[];
  onOrderNow: (editedItems: GroceryItem[]) => void;
...
export function GroceryList({
  ...
  // Handle order with edited items
  const handleOrderNow = () => {
    const finalItems = allActiveItems.map(item => ({
      name: item.name,
  ...
  const renderOrderButton = () => {
    ...
    if (!isAuthenticated) {
      return (
        <button
          onClick={onLogin}
          className="w-full py-4 px-4 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-xl font-semibold hover:from-teal-700 hover:to-teal-800 transition-all shadow-lg shadow-teal-500/25 flex items-center justify-center gap-3"
        >
    ...
    // Logged in but Swiggy not connected
    if (!isSwiggyConnected) {
      return (
        <button
          onClick={onConnectSwiggy}
          className="w-full py-4 px-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-semibold hover:from-orange-600 hover:to-orange-700 transition-all shadow-lg shadow-orange-500/25 flex items-center justify-center gap-3"
        >
    ...
    return (
      <button
        ...
        className="w-full py-4 px-4 bg-gradient-to-r from-teal-600 to-teal-700 text-white rounded-xl font-semibold hover:from-teal-700 hover:to-teal-800 transition-all shadow-lg shadow-teal-500/25 flex items-center justify-center gap-3 group"
      >
        ...
        <span className="text-lg">Review & Order on Swiggy</span>
      </button>
    );
  };
  ...
  const getSubtext = () => {
    if (!isAuthenticated) {
      return 'Sign in to order groceries directly from MealMate';
    }
    if (!isSwiggyConnected) {
      return 'Connect your Swiggy account to enable one-click ordering';
    }
    return 'Review your list, then we\'ll add items to your Swiggy cart';
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      {/* Order Section */}
      <div className="p-4 border-t border-gray-100 bg-gradient-to-b from-gray-50 to-white tour-order-btn">
        {/* Status Indicator */}
        {isAuthenticated && (
          <div className="flex items-center justify-center gap-4 mb-4">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isAuthenticated ? 'bg-teal-500' : 'bg-gray-300'}`} />
              <span className="text-xs text-gray-500">Logged In</span>
            </div>
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isSwiggyConnected ? 'bg-teal-500' : 'bg-gray-300'}`} />
              <span className="text-xs text-gray-500">Swiggy Connected</span>
            </div>
          </div>
        )}
